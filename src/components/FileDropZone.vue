...
import { acceptsDroppedFile, exceedsDroppedFileLimit, filesWithinDropBudget, formatDropFileSize, mergeFileSelection } from '@/lib/file-drop-policy'
...
function update(next:File[]){if(props.disabled)return;if(!next.length)return;const compatible=next.filter(accepted);const perFileValid=compatible.filter(file=>!exceedsDroppedFileLimit(file,props.maxFileBytes));if(compatible.length!==next.length)emit('error',`${next.length-compatible.length} 个文件类型不受当前工具支持。`);if(perFileValid.length!==compatible.length)emit('error',`${compatible.length-perFileValid.length} 个文件超过 ${formatDropFileSize(props.maxFileBytes||0)}，未载入内存。`);if(!props.multiple){emit('update:modelValue',perFileValid.slice(0,1));return}const limit=Math.max(1,Math.trunc(props.maxFiles));const merged=mergeFileSelection(files.value,perFileValid,{maxFiles:limit,maxTotalBytes:props.maxTotalBytes});if(merged.rejectedOverBudget)emit('error',`${merged.rejectedOverBudget} 个文件会使本次输入超过 ${formatDropFileSize(props.maxTotalBytes||0)}，未载入内存。`);if(merged.rejectedOverCount)emit('error',`一次最多选择 ${limit} 个文件，其余 ${merged.rejectedOverCount} 个文件未载入。`);emit('update:modelValue',merged.files)}
function receive(list:FileList|File[]){if(!props.disabled)update(Array.from(list))}function dragEnter(){if(!props.disabled)active.value=true}function choose(event:Event){const target=event.target as HTMLInputElement;if(!props.disabled)receive(target.files||[]);target.value=''}function drop(event:DragEvent){active.value=false;if(props.desktopPathOnly&&isDesktop())return;if(!props.disabled&&event.dataTransfer?.files.length)receive(event.dataTransfer.files)}function remove(index:number){if(!props.disabled)emit('update:modelValue',files.value.filter((_,i)=>i!==index))}function clear(){if(!props.disabled)emit('update:modelValue',[]);if(input.value)input.value.value=''}
