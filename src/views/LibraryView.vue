...
function action(source: Source, id: string) {
  const anchor = { sourceId: source.id, pageIndex: selectedPage.value, bbox: selectedBbox.value ?? [0, 0, 1, 1] as [number, number, number, number] }
    ...
    <div class="library-layout">
      ...
      <section class="source-detail panel">
        <template v-if="selected"><div class="detail-title"><div><p class="eyebrow">{{ selected.kind.toUpperCase() }}</p><h3>{{ selected.name }}</h3></div><span class="hash" :title="selected.sha256">#{{ selected.sha256?.slice(0, 8) }}</span></div>
          <div class="source-preview"><SourceCanvas :source="selected" @select="(bbox) => { selectedBbox = bbox; notice = '已记录选区。创建错题后可从卡片回到这个位置。' }" @page="(page) => selectedPage = page" /></div>
