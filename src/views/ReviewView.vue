...
async function rate(rating: ReviewRating) { if (!current.value || ratingInProgress.value) return; ratingInProgress.value = true; try { await store.gradeDocument(current.value.id, rating); revealed.value = false; if (index.value >= queue.value.length - 1) index.value = 0 } finally { ratingInProgress.value = false } }
</script>
