...
/**
 * The toolbox itself: the home route shows every category, `/c/:category`
 * shows one. Both are the same grid, which keeps the transition between
 * "browsing everything" and "browsing PDF" free of layout surprise.
 */
...
<template>
  <div class="mx-auto w-full max-w-300 px-8 py-7">
    ...
    <section v-if="!activeCategory && !query && recentTools.length" class="mb-8">
    ...
    <section v-for="section in filtered" :key="section.id" :class="`cat-${section.accent}`" class="mb-8">
      ...
      <div class="grid gap-2.5 grid-cols-[repeat(auto-fill,minmax(15rem,1fr))]">
        <RouterLink
          ...
          :key="tool.id"
          :to="tool.to"
          class="group relative stack gap-1 p-3.5 rounded-md bg-surface border border-line overflow-hidden
