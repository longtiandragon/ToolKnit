...
describe('media operation catalog', () => {
  ...
  it('only offers operations supported by the detected tracks', () => {
    ...
    expect(mediaOperationAvailable(mediaOperations.find((item) => item.id === 'denoise-audio')!, { ...video, audioCodec: 'aac' })).toBe(true)
    expect(mediaOperationAvailable(mediaOperations.find((item) => item.id === 'make-gif')!, video)).toBe(true)
    expect(mediaOperationAvailable(mediaOperations.find((item) => item.id === 'make-webp')!, video)).toBe(true)
  ...
  it('recognizes supported desktop drops and output MIME types', () => {
    ...
    expect(mediaOutputMime('clip.gif')).toBe('image/gif')
    expect(mediaOutputMime('clip.webp')).toBe('image/webp')
  })
})
