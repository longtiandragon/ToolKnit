...
export async function transcodeDesktopMedia(request: { inputPath: string; outputDir: string; operation: 'extract-mp3' | 'transcode-m4a' | 'transcode-wav' | 'normalize-audio' | 'denoise-audio' | 'transcode-mp4' | 'make-gif' | 'make-webp' | 'mute-video' | 'remove-audio' | 'remove-subtitles' | 'add-subtitle' | 'burn-subtitle' | 'trim-clip' | 'lossless-clip' | 'remux-mp4' | 'extract-subtitle' | 'extract-cover' | 'clean-metadata' | 'edit-chapters'; runId: string; startSeconds?: number; durationSeconds?: number; subtitlePath?: string; chaptersJson?: string }) {
