...
```text
请继续维护当前仓库。先完整阅读 docs\CODEX_HANDOFF_2026-08-16.md；如果当前会话仍能访问用户最初提供的需求附件，也一并阅读，然后检查 git status。不要重置、覆盖或删除当前脏工作区中的现有改动。
...
用户后来明确要求：可以改前端，但必须沿用现有 Knitspace 视觉风格；不要再使用 ui-ux-pro-max；不要使用 subagents。优先解决真实桌面端体验和稳定性，不要为了堆功能再制造孤立页面。
...
- 前端允许修改，但必须**沿用当前 Knitspace 的颜色、字体、密度、组件、右键菜单习惯**；不要突然改成另一套风格。
- 用户明确要求**不要使用 `ui-ux-pro-max`**，因为此前结果不满意。即使早期目标文本写着使用该 skill，后续用户指令优先。
- 用户明确要求**不要使用 subagents**。
...
当前外显产品名大多是 **Knitspace**；但 Rust 凭据库服务名称和部分内部历史命名为 **ToolKnit**（例如 keyring service）。在 GitHub 正式发布前需要统一命名策略，但不要在没有全局审计的情况下直接批量替换，因为 Windows 凭据库名称会影响既存 API Key 的读取。
...
1. 先检查当前实现，不要根据旧聊天记忆盲改。
2. 使用 `apply_patch` 改文件；不要用 shell 重定向创建/覆盖源文件。
3. 修改前端时复用现有 `PageHeader`、`SectionCard`、`AppIcon`、菜单定位与键盘 helper；保持现有密度和颜色变量。
...
6. 新增桌面数据必须考虑：迁移、一次性浏览器数据导入、备份/恢复、删除/孤儿附件、FTS 更新、公开版 feature gate。
7. 修改 Rust 迁移时，迁移版本和测试必须同步；不可把处理任务重启恢复放到 `VaultService::open()`。
8. 不使用 subagents；不使用 `ui-ux-pro-max`，除非用户再次明确撤销此前限制。
